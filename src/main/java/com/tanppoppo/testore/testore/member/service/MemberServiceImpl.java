...
public class MemberServiceImpl implements MemberService {
    ...
    public void joinMember(MemberDTO memberDTO) {
        ...
        String token = generateVerificationToken(); // 토큰 생성
        LocalDateTime tokenExpiration = LocalDateTime.now().plusMinutes(5);
    ...
    /**
     * 새 이메일 인증 토큰을 생성하는 메서드
     *
     * @author dhkdtjs1541
     * @param member 회원 엔티티
     * @return 새로 생성된 이메일 인증 토큰
     */
    @Override
    public String generateNewEmailVerificationToken(MemberEntity member) {
        String newToken = generateVerificationToken();
        member.setEmailVerificationToken(newToken);
        member.setTokenExpirationTime(LocalDateTime.now().plusMinutes(5));
        mr.save(member);
        return newToken;
    }
    ...
    public void resendVerificationEmail(String email) {
        ...
        String newToken = generateNewEmailVerificationToken(member);
