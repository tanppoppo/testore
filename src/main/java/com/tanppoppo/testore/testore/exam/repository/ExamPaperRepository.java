...
//    List<ExamPaperEntity> findSortedExamPapersByShareCount(Pageable pageable);
    ...
    @Query("SELECT ep FROM ExamPaperEntity ep JOIN ItemLikeEntity le ON le.itemId = ep.examPaperId WHERE le.memberId.memberId = :memberId and ep.title LIKE %:keyword% and le.itemType = :itemType and ep.publicOption = true order by ep.examPaperId desc")
    ...
    @Query("SELECT ep FROM ExamPaperEntity ep JOIN BookmarkEntity bm ON bm.itemId = ep.examPaperId WHERE bm.memberId.memberId = :memberId and ep.title LIKE %:keyword% and bm.itemType = :itemType order by ep.examPaperId desc")
